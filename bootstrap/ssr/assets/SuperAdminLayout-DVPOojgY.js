...
function SuperAdminLayout({ children, header, breadcrumbs }) {
  ...
  useEffect(() => {
    ...
    if (echo) {
      ...
      echo.channel("superadmin-updates").listen(".tenant.created", (e) => {
      ...
      const conn = echo.connector?.ably?.connection;
      ...
      const ch = echo.connector?.ably?.channels?.get("superadmin-updates");
      if (ch) {
        ch.subscribe((msg) => {
