...
function AdminNavbar({ title, breadcrumbs }) {
  ...
  useEffect(() => {
    ...
    if (echo && currentTenant?.id) {
      echo.channel(`tenant-updates.${currentTenant.id}`).listen(".invoice.generated", (e) => {
...
function AdminLayout({
  ...
  useEffect(() => {
    const echo = getEcho();
    if (echo && currentTenant?.id) {
      echo.channel(`tenant.${currentTenant.id}.reservations`).listen(".reservation.created", (e) => {
    ...
    return () => {
      try {
        const echo2 = getEcho();
        if (echo2 && currentTenant?.id && echo2.connector?.ably?.connection?.state === "connected") {
          echo2.leave(`tenant.${currentTenant.id}.reservations`);
        }
      } catch {
      }
    };
